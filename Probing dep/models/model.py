...
class multiTaskNetwork(nn.Module):
    def __init__(self, params):
        ...
        self.attention = nn.MultiheadAttention(self.hiddenSize, num_heads=8)
        self.feedForward = nn.Sequential(
            nn.Linear(self.hiddenSize, 2048),
            nn.ReLU(),
            nn.Linear(2048, self.hiddenSize)
        )
        self.layerNorm1 = nn.LayerNorm(self.hiddenSize)
        self.layerNorm2 = nn.LayerNorm(self.hiddenSize)
    ...
    def forward(self, tokenIds, typeIds, attentionMasks, taskId, taskName):
        ...
        # num_layers = 12
        # sequenceOutput = hiddenStates[num_layers]
        
        # Apply multi-head attention and feed-forward
        attnOutput, _ = self.attention(sequenceOutput, sequenceOutput, sequenceOutput)
        attnOutput = self.layerNorm1(sequenceOutput + attnOutput)
        ffOutput = self.feedForward(attnOutput)
        sequenceOutput = self.layerNorm2(attnOutput + ffOutput)
        
        # concat the four last layers
        # sequenceOutput = torch.cat([hiddenStates[i] for i in [-1, -2, -3, -4]], dim = 0)
        if len(outputs) > 1:
        ...
        # Apply additional attention and feed-forward layers to task-specific output
        sequenceOutput = sequenceOutput.transpose(0, 1)
        attnOutput, _ = self.attention(sequenceOutput, sequenceOutput, sequenceOutput)
        attnOutput = self.layerNorm1(sequenceOutput + attnOutput)
        ffOutput = self.feedForward(attnOutput)
        sequenceOutput = self.layerNorm2(attnOutput + ffOutput)
...
class multiTaskModel:
    ...
    def predict_step(self, batchMetaData, batchData):
        ...
        print("Pred model logits shape: {}".format(outLogits.size()))
