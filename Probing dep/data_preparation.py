...
def load_data(dataPath, taskType, hasLabels):
    ...
    for line in open(dataPath):
        ...
        if taskType == TaskType.SingleSenClassification:
            ...
            if hasLabels is True or hasLabels == 'True':
        ...
        with tqdm(total = len(data), position = chunkNumber) as progress:  
            for idx, sample in enumerate(data):
                ...
                for word, label in zip(sample['sentence'], sample['label']):
                    ...
                    # print(f'Word: {word} and Tokens: {tokens}')
                ...
                print('number item in labelMap: ', len(labelMap))
                
                labelMapTemp = []
                for l in tempLabels:
                    if l not in labelMap.keys():
                        labelMapTemp.append(labelMap['O'])
                    else:
                        labelMapTemp.append(labelMap[l])
                

                tempLabelsEnc = pad_sequences([labelMapTemp], 
...
def create_data_ner_new(data, wrtPath, maxSeqLen, tokenizer, labelMap):
    with open(wrtPath, 'w') as wf:
        for idx, sample in enumerate(data):
            ids = sample['uid']
            tempTokens = ['[CLS]']
            tempLabels = ['[CLS]']
